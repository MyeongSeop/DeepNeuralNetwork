...
model.add_layer('FC-1', FCLayer(784, 100))
model.add_layer('Tanh1', ReLU())
model.add_layer('FC-6', FCLayer(100, 10))
model.add_layer('Softmax Layer', SoftmaxLayer())
# epoch 50 lr 0.05 reg 1e-3 model (784, 100) relu (100, 10) -> 0.87
# epoch 100 lr 0.01 reg 0.001 (784, 256) Tanh (256, 128) Tanh (128, 64) Tanh (64, 10) -> 0.88 / 0.88
# epoch 100 lr 0.01 reg 0.001 (784, 512) ReLU (512, 256) ReLU (256, 128) ReLU (128, 64) ReLU (64, 10) -> 0.88 / 0.88

# Add layers - example 2
# RELU : 0.87, Sigmoid : 0.6, Tanh : 0.89
'''
model.add_layer('FC-1', FCLayer(784, 500))
model.add_layer('ReLU1', Tanh())
model.add_layer('FC-2', FCLayer(500, 500))
model.add_layer('ReLU2', Tanh())
